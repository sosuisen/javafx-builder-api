...
public record PropertyMethodModel(
    ...
    public static class Builder {
        ...
        public PropertyMethodModel build() {
            ...
            String methodBaseName = propertyName.substring(0, propertyName.length() - 8); // Remove
                                                                                          // "Property",
                                                                                          // e.g.,
                                                                                          // "text"
            ...
            String propertyType =
                propertyMethod.getGenericReturnType().getTypeName().replace("$", ".");
            // Check for type replacement
            propertyType = TypeMappingManager
                .getReplacement(classMetadata.getCanonicalClassName(), propertyType);

            String methodAnnotation = MethodAnnotationManager.getMethodAnnotation(
                classMetadata.getCanonicalClassName(), propertyName
            );

            String declareingClassname = propertyMethod.getDeclaringClass().getCanonicalName();
